...
1) име на районa; 2) брой гласоподаватели за района; 3) брой партии, за които се гласува; 
...
int main()
{
    ...
    if (!mainWriter) {
        ...
        /* Записът на файлът не е сортиран (нито по проценти, нито по азбучен ред) 
        с идеята да бъде видна разликата между сортирания му вид (показан на конзолата) 
        и първоначалния му такъв (предоставен в началото и съответно написан във файла). 
        */
    ...
    // Операция на сортиране по имената на район след записването във файла.
    ...
    for (const auto& district : districts) {
        // Интегрирана сортирофка на резултатите на база процентен резултат
